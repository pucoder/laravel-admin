        ...
        @foreach($options as $option => $label)
            ...
            {!! $inline ? '<span class="icheck">' : '<div class="checkbox icheck">' !!}
                ...
                <label @if($inline)class="checkbox-inline"@endif>
                    <input type="checkbox" name="{{$name}}[]" value="{{$option}}" class="{{$class}}" {{ false !== array_search($option, old($column, $value ?? [])) || ($value === null && in_array($option, $checked)) ?'checked':'' }} {!! $attributes !!} />&nbsp;{{$label}}&nbsp;&nbsp;
                </label>
